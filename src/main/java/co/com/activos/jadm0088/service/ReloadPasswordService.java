    ...
    private void saveNewPasswordToHistory(String userName, String hashedPassword) {
        try (Connection connection = OracleFactory.getConexion()) {
            ...
            String insertHistory
                    = "INSERT INTO ADM.PASSWORD_HISTORY (user_id, hashed_password, fecha_cambio) "
                    + "VALUES (?, ?, SYSDATE)";

            PreparedStatement psHistory = connection.prepareStatement(insertHistory);
            psHistory.setString(1, userName);
            psHistory.setString(2, hashedPassword);
            psHistory.executeUpdate();

            String updateUser
                    = "UPDATE ADM.USUARIOS "
                    + "SET USU_PRIMER_LOGIN = 'N' "
                    + "WHERE USU_USUARIO = ?";

            PreparedStatement psUpdate = connection.prepareStatement(updateUser);
